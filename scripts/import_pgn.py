...
def slugify(event: str) -> str:
    """Descriptive filename slug from a CCC event name.

    Drops the leading ``CCC `` label and any trailing ``(time control)`` group,
    lowercases, and reduces everything else to hyphen-separated words, e.g.
    ``CCC 26 Bullet: Semifinals`` -> ``26-bullet-semifinals`` and
    ``CCC 22 Rapid: Tiebreaker #1 (10|3)`` -> ``22-rapid-tiebreaker-1``.
    """
    name = re.sub(r'\s*\([^()]*\)\s*$', '', event).strip()
    name = re.sub(r'(?i)^ccc\s+', '', name)
    name = name.lower().replace('#', '')
    name = re.sub(r'[^a-z0-9]+', '-', name).strip('-')
    if not name:
        raise ValueError(f'Event name produced an empty slug: {event!r}')
    return name
...
def prepare_pgn(content: bytes) -> tuple[bytes, dict, int]:
    ...
    # Filename is the start date plus a descriptive slug from the event name; the
    # end date and full event name stay in metadata. A collision suffix (a/b/c…)
    # is assigned in main() only when start date AND slug both repeat.
    stem = f'ccc_{iso_date(start)}_{slugify(event)}'
...
def main() -> None:
    ...
    parser.add_argument('--rebuild', action='store_true',
                        help='Build the whole archive from the given sources, replacing every existing ZIP.')
    args = parser.parse_args()
    entries = [] if args.rebuild else json.loads((ROOT / 'ccc_manifest.json').read_text(encoding='utf-8'))
    ...
    for source in args.files:
        ...
        # Collision suffix (a/b/c…) only when start date AND slug both repeat.
        base_stem = entry['zip'][:-4]
        for suffix in ('', *(chr(c) for c in range(ord('a'), ord('z') + 1))):
            candidate = f'{base_stem}{suffix}.zip'
            if candidate not in known:
                break
        else:
            raise ValueError(f'Too many CCC events sharing {base_stem} to disambiguate.')
        entry['pgn'] = f'{base_stem}{suffix}.pgn'
        entry['zip'] = candidate
        destination = ROOT / str(entry['year']) / entry['zip']
        if not args.rebuild and destination.exists():
            raise ValueError(f"Archive already exists: {destination}")
        if not args.rebuild and any(
                old['event'].strip() == entry['event'] and old['start'] <= entry['end']
    ...
    if not args.write:
        ...
        return
    if args.rebuild:
        for stale in sorted(ROOT.glob('20[0-9][0-9]/*.zip')):
            stale.unlink()
